...
def matrix_mul(matrix_a, matrix_b):
    ...
    Raises:
        TypeError: If either matrix_a or matrix_b
        is not a list of lists of ints/floats.
        TypeError: If either matrix_a or matrix_b is empty.
        TypeError: If either matrix_a or matrix_b has different-sized row.
    ...
    if not all((isinstance(item, int) or isinstance(item, float))
               for item in [num for row in matrix_a for num in row]):
        raise TypeError("matrix_a should contain only integers or floats")
    if not all((isinstance(item, int) or isinstance(item, float))
               for item in [num for row in matrix_b for num in row]):
    ...
    inverted_b = []
    for numb in range(len(matrix_b[0])):
        new_row = []
        for n in range(len(matrix_b)):
            new_row.append(matrix_b[n][numb])
